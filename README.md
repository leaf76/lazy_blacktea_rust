...
The output installer/bundle is generated by Tauri under `src-tauri/target/` depending on your OS.

## Usage Notes

- Make sure at least one Android device is connected and visible via `adb devices`.
- If `adb` is not in `PATH`, set an absolute ADB path in the app **Settings**.
- Wireless pairing is supported through the app pairing flow (Android 11+ typically required).
- Some features may require additional device permissions (e.g., file operations).
...
3. On Linux, you may need udev rules to access USB devices.
4. On Windows, make sure the correct OEM USB driver is installed.
